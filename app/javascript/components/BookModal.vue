<template>
  <v-container>
      ...
      <v-col cols="12" xs="12" md="9">
        ...
        <div v-show="!show">
          ...
          <div v-if="HasImpression">
            <v-btn text color="primary" @click="ShowCreate = !ShowCreate">{{
              CreateImpressionText
            }}</v-btn>
            <form
              v-if="ShowCreate"
              @submit.prevent="CreateImpression"
          ...
          <div v-else>
            <v-row>
              <v-btn text color="primary" @click="ShowEdit = !ShowEdit">{{
                EditImpressionText
              }}</v-btn>
              <v-btn text color="error" @click="DestroyImpression(ImpressionId)"
                >感想を削除する</v-btn
              >
            </v-row>
            <form
              v-if="ShowEdit"
              @submit.prevent="EditImpression(ImpressionId)"
              enctype="multipart/form-data"
              accept-charset="UTF-8"
            >
              <v-col class="pb-0" cols="12">
    ...
    >
    <v-btn
      class="ml-1 mt-2 font-weight-medium"
      color="error"
      @click="DeleteBook(BookInfo.id)"
      >本棚から削除</v-btn
    >
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ChangeTag(tag) {
      axios.post("/books/change_tag/" + this.BookInfo.id, {
        book: { tag_id: tag },
      });
      this.$store.state.books.find(
        (element) => element.id == this.BookInfo.id
      ).tag_id = tag;
    },
    EditImpression(impression_id) {
      axios.patch("/impressions/" + impression_id, {
        impression: { content: this.ImpressionContent },
      });
      this.$store.state.impressions.find(
        (item) => item.id === impression_id
      ).content = this.ImpressionContent;
      this.ShowEdit = false;
    ...
    CreateImpression() {
      ...
      axios
        .post("/impressions", {
          impression: {
            content: this.NewImpression,
            book_id: this.BookInfo.id,
          },
        })
        .then(function () {
          axios.get("/impressions").then(function (response) {
    ...
    DestroyImpression(impression_id) {
      axios.delete("/impressions/" + impression_id);
      // store内から対象の感想を探して削除する
      const checkDeleteImpression = (element) => element.id == impression_id;
      var index = this.$store.state.impressions.findIndex(
        checkDeleteImpression
      );
