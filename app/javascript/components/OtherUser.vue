      ...
      <v-row class="brown lighten-5">
        ...
        >
          <a @click.stop="dialog = true" @click="DisplayBook(book)"
            ><img class="text-center" :src="book.image"
          /></a>
        </v-col>
      </v-row>
      <template>
              ...
              <v-container>
                ...
                <v-row>
                  ...
                  <v-col cols="12" xs="12" md="9">
                    <p v-if="ImpressionContent(BookInfo.id)" class="caption">
                      感想
                    </p>
  ...
  computed: {
    ...
    books() {
      return function (id) {
        return id == 0
          ? this.user_books
          : this.user_books.filter(function (value) {
    ...
    ImpressionContent() {
      return function (id) {
        let ImpressionChoose = this.impressions.find(
